...
int open_fd(int pcli, int pflags, mode_t pmode, int *pfd)
{
  ...
  snprintf(fn, MAX_NAMELEN - 1,
           "client%d.fifo", pcli);
  fprintf(stderr, "DEBUG(open_fd:%zu): try to open %s\n",
          mypid, fn);
 open_fd_try:
  ...
  if (fd == -1)
    {
      if (errno == EINTR)
        ...
        {
          fprintf(stderr,
                  "DEBUG(open_fd:%zu): open of %s would block\n",
                  mypid, fn);
          return 1;
        }
      else
        {
          fprintf(stderr,
                  "Error: open_fd: opening of %s failed: %s(%d)\n",
  ...
  else
    {
      *pfd = fd;
      fprintf(stderr,
              "DEBUG(open_fd:%zu): open of %s OK, fd is %d\n",
              mypid, fn, *pfd);
      return 0;
    }
}
...
int write_fd(int pfd, char* pbuf, ssize_t psiz)
{
 ...
 write_fd_try:
  ...
  if (nfd == psiz)
    {
      fprintf(stderr,
              "DEBUG(write_fd:%zu): Wrote OK to %d\n",
              mypid, pfd);
      retval = 0;
    }
  else if (nfd == -1)
    {
      if (errno == EINTR)
        ...
        {
          fprintf(stderr,
                  "DEBUG(write_fd:%zu): %d would block\n",
                  mypid, pfd);
          retval = 1;
        }
      else
...
int read_fd(int pfd, char* pbuf, ssize_t pbufsize, ssize_t *prd)
{
 ...
 read_fd_try:  
  ...
  if (res > 0)
    {
      /*      
      fprintf(stderr,"DEBUG(read_fd:%zu): read %zu from %d\n",
              mypid, res, pfd);
      */
      *prd = res;
    }
  else if (res == -1)
    {
      if (errno == EINTR)
        ...
        {
          fprintf(stderr,"DEBUG(read_fd:%zu): reading would block\n",
                  mypid);
          return 1;
        }
      else
        {
          fprintf(stderr,"Error: read_fd: reading failed in %zu: %s(%d)\n",
